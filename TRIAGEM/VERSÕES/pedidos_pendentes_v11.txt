...
// 1. IMPORTAÇÕES
// ==============================================================================
import 'dart:typed_data'; // Para Uint8List (usado em imagens no PDF)
// -------------------------
// Flutter SDK
// -------------------------
...
// -------------------------
import 'dart:io'; // Para manipular arquivos (File)
import 'dart:convert'; // Para base64Encode (usado em PDF web)
import 'package:flutter/foundation.dart'
    show kIsWeb; // Para detectar ambiente web
...
class _PedidosEntreguesScreenState extends State<PedidosEntreguesScreen> {
  ...
  Future<void> _gerarRecibo() async {
    ...
    try {
      ...
      for (final pedido in _pedidos.where(
        (p) => _pedidosSelecionados.contains(p.id),
      )) {
                ...
                child: pw.Column(
                  ...
                  children: [
                    ...
                    if (logoBytes != null && logoBytes.isNotEmpty)
                      ...
                      // Espaço reservado se não houver logo
  ...
  // -------------------------------------------------------------------------
  /// Carrega o logo da loja a partir do campo `loja_logo` no banco
  Future<Uint8List?> _carregarLogoDaLoja(Pedido pedido) async {
    try {
      // Verifica se o campo é uma URL válida
      final url = pedido.lojaLogo;
      if (url == null || url.isEmpty) {
        return null;
      }

      // Baixa a imagem da web
      final response = await http.get(Uri.parse(url));
      if (response.statusCode == 200) {
        return response.bodyBytes; // ✅ Retorna Uint8List?
      } else {
        debugPrint('Erro ao baixar logo: Status ${response.statusCode}');
